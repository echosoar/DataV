...
class DataModuleLibraryManage extends React.Component {
  constructor( props ) {
    super( props );
    this.state = {
      data: [],
  ...
  handleAddThisTemplate() {
    ...
    if(newJsonIsCanUse) {
      ...
      if(!defaultConfig.api || !defaultConfig.api.dataModuleLibraryAdd) {
      ...
      <div className="libraryManageItem-main">
      {
        isOpenNew && <div className="libraryManageItem-main-add">
          ...
          <div className="libraryManageItem-main-add-item">
            <div className="libraryManageItem-main-add-item-title">数据组件配置JSON</div>
            <Input type="textarea" placeholder="请输入数据组件配置JSON" onChange={this.handleAddNewJson.bind(this)} autosize={{ minRows: 20}} />
            <div className="libraryManageItem-main-add-error">
            {
              !newJsonIsCanUse && '请输入正确的JSON字符串，并包含必须字段'
            }
            </div>
            <Button className="libraryManageItem-main-add-button" type="primary" onClick={this.handleAddThisTemplate.bind(this)}>确认添加数据组件</Button>
          </div>
          <div className="libraryManageItem-main-add-item libraryManageItem-main-add-info">
            <div className="libraryManageItem-main-add-item-title">数据组件包含：</div>
            <ul>
              <li>name ：【必须】模块名称。</li>
...
module.exports = connect(mapStateToProps)(DataModuleLibraryManage);
